...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_greater_than(int(response.getheader('content-length')), 141)
        response_bytes = response.read()

        # Compare with block header
        response_header = self.test_rest_request("/headers/1/{}".format(bb_hash), req_type=ReqType.BIN, ret_type=RetType.OBJ)
        assert_equal(int(response_header.getheader('content-length')), 141)
        response_header_bytes = response_header.read()
        assert_equal(response_bytes[:141], response_header_bytes)

        # Check block hex format
        response_hex = self.test_rest_request("/block/{}".format(bb_hash), req_type=ReqType.HEX, ret_type=RetType.OBJ)
        assert_greater_than(int(response_hex.getheader('content-length')), 282)
        response_hex_bytes = response_hex.read().strip(b'\n')
        assert_equal(binascii.hexlify(response_bytes), response_hex_bytes)

        # Compare with hex block header
        response_header_hex = self.test_rest_request("/headers/1/{}".format(bb_hash), req_type=ReqType.HEX, ret_type=RetType.OBJ)
        assert_greater_than(int(response_header_hex.getheader('content-length')), 282)
        response_header_hex_bytes = response_header_hex.read(282)
        assert_equal(binascii.hexlify(response_bytes[:141]), response_header_hex_bytes)
