...
class SegWitTest(BitcoinTestFramework):
    ...
    def test_block_relay(self):
        ...
        if self.segwit_status != 'active':
            ...
            for height in all_heights:
                ...
                assert_equal(block.serialize(with_witness=True, legacy=False), wit_block.serialize(with_witness=True, legacy=False))
            ...
            assert_equal(wit_block.serialize(with_witness=True, legacy=False), hex_str_to_bytes(rpc_block))
            assert_equal(wit_block.serialize(with_witness=False, legacy=False), non_wit_block.serialize(legacy=False))
            assert_equal(wit_block.serialize(with_witness=True, legacy=False), block.serialize(with_witness=True, legacy=False))

            # Test size, vsize, weight
            # Caution: Prefork blocks seralize in legacy way.
            rpc_details = self.nodes[0].getblock(block.hash, True)
            assert_equal(rpc_details["size"], len(block.serialize(with_witness=True, legacy=True)))
            assert_equal(rpc_details["strippedsize"], len(block.serialize(with_witness=False, legacy=True)))
            weight = 3 * len(block.serialize(with_witness=False, legacy=True)) + len(block.serialize(with_witness=True, legacy=True))
    ...
    def test_block_malleability(self):
        ...
        self.nodes[0].submitblock(bytes_to_hex_str(block.serialize(with_witness=True, legacy=False)))
    ...
    def test_witness_block_size(self):
        ...
        # limit
        assert(len(block.serialize(with_witness=True, legacy=False)) > 2 * 1024 * 1024)
    ...
    def test_uncompressed_pubkey(self):
        ...
        sig_hash = SegwitVersion1SignatureHash(script_pubkey, tx5, 0, SIGHASH_ALL | SIGHASH_FORKID, tx4.vout[0].nValue)
        signature = key.sign(sig_hash) + b'\x41'  # 0x41 is SIGHASH_ALL | SIGHASH_FORKID
    ...
    def test_signature_version_1(self):
        ...
        for sigflag in [0, SIGHASH_ANYONECANPAY]:
            for hashtype in [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE]:
                hashtype |= (sigflag | SIGHASH_FORKID)
                block = self.build_next_block()
                tx = CTransaction()
        ...
        for i in temp_utxos:
            ...
            tx.wit.vtxinwit.append(CTxInWitness())
            sign_p2pk_witness_input(witness_program, tx, index, SIGHASH_ALL | SIGHASH_ANYONECANPAY | SIGHASH_FORKID, i.nValue, key)
            index += 1
        block = self.build_next_block()
        self.update_witness_block_with_transactions(block, [tx])
        test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
