 ...
 * between bgoldd, and bitcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-minfinalizationage=<n>", strprintf("Set the minimum amount of time to wait between a block header reception and the block finalization. Unit is seconds. For an accurate finalization state, better to check getfinalizedblockhash to directly see if the block is finalized. (default: %d)", DEFAULT_MIN_FINALIZATION_DELAY), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-minfinalizationdepth=<n>", strprintf("Configure at what depth blocks are considered final (default: %d). Use -1 to disable. For an accurate finalization state, better to check getfinalizedblockhash to directly see if the block is finalized.", DEFAULT_MAX_REORG_DEPTH), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BTCGPU/BTCGPU>";
    const std::string URL_WEBSITE = "<https://bitcoingold.org>";
